...
export const TipoBien = () => {
  ...
  const { idBien, setIdBien, tipoBien, setTipoBien } = useContext(bienContext);

  useEffect(() => {
    const getIdBien = async () => {
      const id = await serverActions.getBienId(tipoBien).then((id) => {
    ...
    <div className="w-full max-w-sm min-w-[200px]">
      <div className="relative">
        <select
          ...
          value={tipoBien}
          onChange={({ target }) => setTipoBien(target.value)}
        >
